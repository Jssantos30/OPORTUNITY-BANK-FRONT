...
const Contacto = () => {
  return (
    <>
      <Banner title="Contáctanos" image_url={`${process.env.NEXT_BASE_PATH}/contacto-banner.png`} />
            ...
            return (
              ...
              >
                <div className="flex justify-between items-center border-b-2 color_gray_5 w-4/5 pb-6">
                  <p className="text-color_primary_2_dark_2 font-semibold w-3/5 text-2xl lg:text-3xl ">
                    {item.title}
                  </p>
                ...
                <div className={`${item.text === 'banco.oportunidades@medellindigital.gov.co' ? 'emailContact' : 'text-xl'}  font-semibold font-section pt-4`}>{item.text}</div>
              </div>
            )
          })}
        </div>
